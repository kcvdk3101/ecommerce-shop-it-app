...
import { config, configFormData } from '../constant';
import {
  LOGIN_REQUEST,
  LOGIN_SUCCESS,
  LOGIN_FAIL,
...
const register = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post('/api/v1/register', userData, configFormData)
    dispatch({
...
const updateProfile = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put('/api/v1/me/update', userData, configFormData)
    dispatch({
